...
class Hand(object):
    ...
    def __init__(self):
        self.curr_path = settings.BASE_DIR +'/Mainapp/handright'
        # print( self.curr_path+'/checkpoints')
        os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'
        self.nn = rnn(
            log_dir=self.curr_path+'logs',
            checkpoint_dir=self.curr_path+'/checkpoints',
            prediction_dir=self.curr_path+'/predictions',
    ...
    def _sample(self, lines, biases=None, styles=None):
        ...
        if styles is not None:
            for i, (cs, style) in enumerate(zip(lines, styles)):
                x_p = np.load(self.curr_path+'/styles/style-{}-strokes.npy'.format(style))
                c_p = np.load(self.curr_path+'/styles/style-{}-chars.npy'.format(style)).tostring().decode('utf-8')
